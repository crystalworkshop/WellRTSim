...
% appendChemistryH5 Append latest chemistry time-series sample to HDF5.
    ...
    h5write(file, '/chemistry/ca_in_kg', state.CaInKg(j), j, 1);
    h5write(file, '/chemistry/ca_out_kg', state.CaOutKg(j), j, 1);
    h5write(file, '/chemistry/ca_precip_kg', state.CaPrecipKg(j), j, 1);
    h5write(file, '/chemistry/ca_fluid_delta_kg', state.CaFluidDeltaKg(j), j, 1);
    h5write(file, '/chemistry/ca_balance_residual_kg', state.CaBalanceResidualKg(j), j, 1);
    state.h5_chem_idx = j;
end
